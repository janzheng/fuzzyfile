...
export async function handleGetFile(config, BUCKET, request) {
  try {
    ...
    console.log('[handleGetFile] fileObject metadata:', {
      key: fileObject?.key,
    ...
    console.log('[handleGetFile] Response metadata:', response.metadata);
    return response;
  } catch (error) {
...
async function fetchFileObject(key, BUCKET, request) {
  try {
    // Request the file with metadata
    ...
    const contentType = fileObject?.httpMetadata?.contentType;
    
    // Handle range requests for both video and audio
    if (range && fileObject && 
        (contentType?.startsWith('video/') || contentType?.startsWith('audio/'))) {
      const startEnd = range?.replace('bytes=', '')?.split('-')?.map(val => parseInt(val));
      const start = startEnd?.[0] || 0;
      const end = startEnd?.[1] || fileObject.size - 1;
      
      if (start > 0 || end < fileObject.size - 1) {
