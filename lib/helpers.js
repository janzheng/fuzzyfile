...
export async function handleListFile(config, BUCKET) {
  ...
  console.log('[handleListFile] pathname:', config.pathname, '>>> prefix:', prefix)
  
  let bucketList = await BUCKET.list({
    prefix,
    include: ['httpMetadata']
  })
  let objects = bucketList.objects.map(o => ({ key: o.key, uploaded: o.uploaded, size: o.size, type: o.httpMetadata?.contentType }))
  while (bucketList.truncated) {
    bucketList = await BUCKET.list({ cursor: bucketList.cursor })
    objects = objects.concat(bucketList.objects.map(o => ({ key: o.key, uploaded: o.uploaded, size: o.size, type: o.httpMetadata?.contentType })))
...
export async function handleAddFile(config, BUCKET, fileBuf) {
  ...
  console.log('[handleAddFile] Using key:', upkey, 'nanoid:', config.nanoid)
  ...
  if(config.returnFile) {
    ...
    if (upkey?.includes('.svg')) { // dumb but works
      headersObj['content-type'] = 'image/svg+xml'
    }
    const headers = new Headers(headersObj);
    fileObject?.writeHttpMetadata(headers);
