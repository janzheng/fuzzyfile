...
import { handleSearchParams, handleAddFile, handleGetFile, handleListFile, handleDownloadFile, handleHashDownload, handleGetFileDetails } from './helpers.js'
import { corsHeaders } from '../handlers/cors-handler.js'
import { handleAirtable } from './airtable.js'
import { getPresignedUrl } from '../handlers/presign-handler.js'
import { downloadFileBuf } from './utils.js'
...
export const getHandler = async (request, BUCKET) => {
  ...
  if (config && config?.cmd == 'add') {
    ...
    let res = await handleGetFile(config, BUCKET, request)
...
export const postHandler = async (request) => {
  ...
  if (request.headers.get('content-type') == "application/json") {
    ...
    if (config && config?.cmd == 'add') {
      ...
      // searches + downloads file via ipfs hash
      let json = await handleGetFileDetails(config, BUCKET)
      return new Response(JSON.stringify(json), {
        headers: corsHeaders
      })
    } else if (config && (config?.cmd == 'presigned' || config?.cmd == 'presign')) {
      // get presigned url
      let json = await getPresignedUrl(config)
      return new Response(JSON.stringify(json), {
        headers: corsHeaders
      })
